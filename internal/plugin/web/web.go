// Package web embeds the built management UI (a single inlined index.html) and
// serves it as a CPA plugin resource under
// /v0/resource/plugins/key-bind/index.html.
//
// dist/index.html is a build artifact produced by `npm run build` in ../../web.
// A placeholder is committed so the Go build never fails before the frontend is
// built; the real UI replaces it after a frontend build.
package web
